...
const madeSlice = (set, get) => ({
  ...
  fetchMade: async () => {
    try {
      ...
      const normalized = (response.data || []).map((r) => ({
        ...
        image_url: r.image_url || "/img/pexels-enginakyurt-1435904.jpg",
  ...
  fetchTrending: async () => {
    try {
      ...
      // console.log("Normalized trending:", normalized); 
  ...
  logMade: async (recipeId) => {
    try {
      ...
      if (existing) {
        ...
        // add new entry with count 1 (placeholder data; API will fill in title, username, image)
        set((state) => ({
          madeRecipes: [
            ...state.madeRecipes,
            { recipe_id: recipeId, id: recipeId, count: 1 },
          ],
        }));
      }
